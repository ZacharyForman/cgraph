...
public:
  ...
  template<typename RT = T>
  std::enable_if_t<n != 1 && m != 1, RT>
  operator()(std::size_t i) const = delete;

  // Computes the matrix product of two matrices.
  template<typename R, std::size_t o>
  Matrix<std::common_type_t<R,T>, m, o>
  operator*(const Matrix<R, n, o> &mat) const {
  ...
  template<typename R>
  Matrix<std::common_type_t<T, R>, m, n>
  operator+(const Matrix<R, m, n> &mat) const {
  ...
  // "In place" product.
  template<typename R, std::size_t o>
  Matrix<Convertible<R, T, T>, m, o>
  &operator*=(const Matrix<R, n, o> &mat) {
    *this = *this * mat;
    return *this;
  }
  ...
  // In place sum.
  template<typename R>
  Matrix<Convertible<R, T, T>, m, n>
  operator+=(const Matrix<R, m, n> &mat) {
    for (std::size_t i = 0; i < m; i++) {
      for (std::size_t j = 0; j < n; j++) {
  ...
  // Disable for differing types.
  template<typename R>
  Matrix<NotConvertible<R, T, T>, m, n>
  operator*=(const R &val) = delete;
  ...
  template<typename RT = T>
  operator std::enable_if_t<n * m == 1, RT>() { return data[0]; }
  // Other matrices can't decay to a scalar.
  template<typename RT = T>
  operator std::enable_if_t<n * m != 1, RT>() = delete;
...
Matrix<std::common_type_t<R, T>, m, n>
operator*(R val, Matrix<T, m, n> mat) {
...
template<typename T, std::size_t m, std::size_t n, typename R>
Matrix<std::common_type_t<R, T>, m, n>
operator*(Matrix<T, m, n> mat, R val) {
  return val * mat;
}
